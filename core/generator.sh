...
#   ZET_SKILLS_MINIMAL — minimal-profile (scope: universal only) skill output dir (unset = feature off)
#
# SCOPE TAGGING (a session-context filter, independent of backend tagging):
#   A template can set `scope: universal|vault` (default: vault — every existing template that never
#   sets it is excluded from the minimal profile; the safe default, since a skill wrongly excluded is a
#   minor inconvenience, a skill wrongly included defeats the profile's purpose). `scope: universal`
#   mirrors the skill into [paths].skills-minimal IF that path is configured, using the SAME model as
#   the full skill (a discovery filter, not a model-tier swap). A minimal-profile launch (`ccm` alias)
#   points --plugin-dir at skills-minimal so a session outside the vault/personal-system context
#   discovers only repo-agnostic skills instead of the full set.
#
...
# MINIMAL-PROFILE skill output — mirrors ONLY `scope: universal` skills into a trimmed dir for a session
# outside the vault/personal-system context (e.g. an unrelated repo). This is an EXCLUSION set (opposite of
# skills-local's dual-tier default): a skill mirrors here ONLY when explicitly tagged `scope: universal`; the
# default (`scope` unset = vault-scoped) does NOT appear here. NO default path, same reasoning as
# skills-codex above — an unconfigured minimal profile must be a no-op, never silently write somewhere
# unintended. Empty/unset ⇒ feature is off ⇒ scope: universal is a no-op.
_skills_minimal_raw="${ZET_SKILLS_MINIMAL:-$(zet_config_get "paths" "skills-minimal" "")}"
SKILLS_MINIMAL_DIR=""
[ -n "$_skills_minimal_raw" ] && SKILLS_MINIMAL_DIR="$(resolve_path "$_skills_minimal_raw")"
...
# Same guard, extended to the minimal-profile plugin dir — it does the same wipe-then-write as skills-local
# (below), so a misconfig pointing it at the full skills dir is just as destructive.
if [ -n "$SKILLS_MINIMAL_DIR" ]; then
    SKILLS_MINIMAL_SKILLS_DIR="$SKILLS_MINIMAL_DIR/skills"
    _phys_minimal=$(_canon_path "$SKILLS_MINIMAL_DIR")
    _phys_minimal_skills=$(_canon_path "$SKILLS_MINIMAL_SKILLS_DIR")
    if [ -n "$_phys_full" ] && { [ "$_phys_full" = "$_phys_minimal" ] || [ "$_phys_full" = "$_phys_minimal_skills" ]; }; then
        echo "ERROR: skills-minimal (or its skills/ subdir) resolves to the full skills dir ($_phys_full)." >&2
        echo "  Refusing to run — the minimal-set wipe would delete the full skill set. Fix [paths].skills-minimal." >&2
        exit 1
    fi
fi
...
# Minimal-profile set is ALSO a plugin (same --plugin-dir discovery mechanism as skills-local) so a
# session outside the vault/personal-system context can load it via `ccm` (Claude Code Minimal). Same
# wipe-then-regenerate reasoning: a skill retagged universal→vault must never leave a stale copy behind.
if [ -n "$SKILLS_MINIMAL_DIR" ] && ! $DRY_RUN; then
    ensure_dir "$SKILLS_MINIMAL_SKILLS_DIR"
    find "$SKILLS_MINIMAL_SKILLS_DIR" -mindepth 1 -maxdepth 2 -name SKILL.md -delete 2>/dev/null || true
    find "$SKILLS_MINIMAL_SKILLS_DIR" -mindepth 1 -maxdepth 1 -type d -empty -delete 2>/dev/null || true
    ensure_dir "$SKILLS_MINIMAL_DIR/.claude-plugin"
    cat > "$SKILLS_MINIMAL_DIR/.claude-plugin/plugin.json" <<'PLUGINJSON'
{
  "name": "ccm-minimal",
  "version": "0.1.0",
  "description": "Universal-scope skills (scope: universal) for a minimal-profile session outside the vault/personal-system context — generated by zet, do not edit."
}
PLUGINJSON
fi
...
for file in "$TEMPLATE_DIR"/*_prompt_template.md; do
    ...
    case "$type" in
        skill)
            ...
            # SCOPE: whether this skill is safe to surface in a MINIMAL profile (a session outside the
            # vault/personal-system context). Default "vault" — every existing template that never sets
            # this field is excluded from the minimal set (the safe direction: a skill wrongly excluded is
            # a minor inconvenience next time; a skill wrongly INCLUDED defeats the point of a trimmed
            # profile but causes no breakage either way — still, default to the narrower set).
            # `scope: universal` is the explicit opt-IN for a skill that's genuinely repo-agnostic.
            scope=$(get_frontmatter_value "$file" "scope")
            [ -z "$scope" ] && scope="vault"
            case "$scope" in
                universal|vault) ;;
                *)
                    echo "  WARNING: $filename — unknown scope '$scope', treating as 'vault'" >&2
                    scope="vault"
                    ;;
            esac
            ...
            # MINIMAL-PROFILE set: only skills marked `scope: universal` mirror into SKILLS_MINIMAL_DIR,
            # and only when that path is actually configured (unset ⇒ feature off, see SKILLS_MINIMAL_DIR
            # above). Uses the SAME model as the full skill — this isn't a model-tier swap like
            # skills-local, just a discovery/path filter, so no model: remapping is needed.
            if [ "$scope" = "universal" ] && [ -n "$SKILLS_MINIMAL_DIR" ] && ! $DRY_RUN; then
                minimal_skill_dir="$SKILLS_MINIMAL_SKILLS_DIR/$name"
                mkdir -p "$minimal_skill_dir"
                {
                    echo "---"
                    echo "name: $name"
                    echo "description: $desc"
                    ...
                    [ -n "$skill_tags" ] && echo "tags: $skill_tags"
                    echo "---"
                    echo "<!-- Generated by Zet from $filename (minimal profile) — do not edit directly -->"
                    echo "<!-- Regenerate: zet generate -->"
                    echo ""
                    printf 'follow %s\n' "$TEMPLATE_DIR/$filename"
                    if [ -n "$prompt_extra" ]; then
                    ...
                    echo ""
                    echo "Read the template file first, then execute its instructions completely."
                } > "$minimal_skill_dir/SKILL.md"
                $QUIET || echo "  skill: $name (+minimal)"
            fi
            ;;
