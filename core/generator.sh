...
# Codex output is a plain additive copy dir (no wipe-then-write step like skills-local's plugin
...
if [ -n "$SKILLS_CODEX_DIR" ]; then
    ...
    # Wipe-then-regenerate, same reasoning as the local-tier set below: a skill retagged with
    # codex:false still matches type: skill, so cleanup_stale's "no matching template" check would
    # never catch it — only a full wipe before rebuilding guarantees no stale codex copy survives.
    if ! $DRY_RUN; then
...
for file in "$TEMPLATE_DIR"/*_prompt_template.md; do
    ...
    case "$type" in
        skill)
            ...
            if [ "$codex_enabled" != "false" ] && [ -n "$SKILLS_CODEX_DIR" ] && ! $DRY_RUN; then
                ...
                mkdir -p "$codex_skill_dir"
                {
                    ...
                    echo "description: $(yaml_quote "$desc")"
                    echo "---"
                    echo "<!-- Generated by Zet from $filename (Codex projection) — do not edit directly -->"
                    echo "<!-- Regenerate: zet generate -->"
                    echo ""
                    printf 'follow %s\n' "$TEMPLATE_DIR/$filename"
                    if [ -n "$prompt_extra" ]; then
...
if [ -n "$AGENTS_STD_DIR$SKILLS_CODEX_DIR" ] && ! $DRY_RUN && [ -d "$SKILLS_DIR" ]; then
    for skill_dir in "$SKILLS_DIR"/*/; do
        ...
        if [ -n "$AGENTS_STD_DIR" ]; then
            ...
            cp "$hw_skill_md" "$AGENTS_STD_DIR/$hw_name/SKILL.md"
